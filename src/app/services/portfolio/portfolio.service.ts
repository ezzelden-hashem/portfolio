...
export class PortfolioService {
  ...
  getCategoryFolders(categoryName: string): FolderInfo[]
  {
    ...
    const findResult = this.portfolioObject.find((category) => category.name === categoryName);
    if (findResult)
    {
  ...
  getFolderProjects(folderId: string): ProjectInfo[]
  {
    ...
    for (const category of this.portfolioObject)
    {
      const findResult = category.folders.find(folder => folder.id === folderId)
      if (findResult)
      {
        folder = findResult;
        break;
  ...
  getProjectById(projectId: string): Project | null
  {
    ...
    .find(p => p.id === projectId) || this.portfolioObject
  ...
  {
    const findResult = this.portfolioObject.find(c => c.name === categoryName)
    if (findResult) {
      return {
        id: findResult.id,
  ...
  getFolderInfoByName(categoryName: string, folderName: string): FolderInfo | null
  {
    const findResult = this.getCategoryFolders(categoryName).find(f => f.name === folderName);
    if (findResult) {
      return {
        id: findResult.id,
        name: findResult.name,
  ...
  getProjectInfoByName(categoryName: string, projectName: string, folderName: string | null = null): ProjectInfo | null
  {
    const findResult = (folderName) ? this.portfolioObject
    .find(c => c.name === categoryName)?.folders
    .find(f => f.name === folderName)?.projects
    .find(p => p.name === projectName)
    : this.portfolioObject
    .find(c => c.name === categoryName)?.projects
    .find(p => p.name === projectName);
    if (findResult) {
      return {
        id: findResult.id,
  ...
  getProjectsFolderByName(categoryName: string, folderName: string): ProjectInfo[] | null
  {
